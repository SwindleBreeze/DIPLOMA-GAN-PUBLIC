...
let generationDeathData = {};

let populationData = {};

let generatedGroundGAN;
listener.BeginContact = function(contact) {
  ...
  if (contact.GetFixtureA().GetBody().GetUserData().id == "head") {
    if (contact.GetFixtureB().GetBody().GetUserData().id == "ground") {
      ...
      // console log x and y of car 
      // console.log("IM FUCKEN DEAD")
      
      console.log(car.person.body);
  ...
  if (contact.GetFixtureB().GetBody().GetUserData().id == "head") {
    if (contact.GetFixtureA().GetBody().GetUserData().id == "ground") {
      ...
      // console.log("IM FUCKEN DEAD")
      // console.log(car.person.head.body.m_xf.position);
      if(generationDeathData[population.gen] == null || generationDeathData[population.gen].length == 0) {
        generationDeathData[population.gen] = [car.person.head.body.m_xf.position]
      } else {
        generationDeathData[population.gen].push(car.person.head.body.m_xf.position)
      }
...
function preload() {
  ...
  grassSprites.push(loadImage("Pics/grass6.png"));

  
  // fetchData('http://localhost:3000/ground?id=1');
  // postData('http://localhost:3000/test', {bangdik: "bam"} )

}

async function generate100Grounds() {
  for (var i = 0; i < 100_000; i++) {
    let tempGround = new Ground();
    tempGround.randomizeGround();
    while (tempGround.isTooSteep()) {
      tempGround = new Ground();
      tempGround.randomizeGround();
    }
    
    // send fetch post request to server
    let response = await fetch('http://localhost:3000/ground', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify(tempGround.generateJsonOfGround())
    });
  }
}


async function saveValidationGround(tempGround) {
  let response = await fetch('http://localhost:3000/finalground',  {
    method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
async function setup() {
  ...
  do {
    console.log("Fetching ground");
    generatedGroundGAN = await fetchData('http://localhost:3000/flatground?id=6');
    console.log("IN THE LOOP: ", generatedGroundGAN);
  } while (generatedGroundGAN == null || generatedGroundGAN.length == 0 || generatedGroundGAN[0].length == 0);
  

  groundTemplate = new Ground();
  // uncomment to create new random ground ---------------------------------
  ...
  // }
  // generatedGroundGAN = []
  // generatedGroundGAN.push(groundTemplate.generateJsonOfGround());
  // -----------------------------------------------------------------------

  // uncomment to create ground from json ---------------------------------
  groundTemplate.generateGroundFromJson(generatedGroundGAN[0])
  // -----------------------------------------------------------------------
  ...
  // postData('http://localhost:3000/test', {bangdik: "cock"} )
...
async function draw() {
  ...
  if (runBest) { // if replaying the best ever game
  ...
  } else if(population) { //if just evolving normally
    if (!population.done()) { //if any players are alive then update them
      ...
      //genetic algorithm
      // grounds[0].show()
      console.log("ITS ALL DEAD");
      await preparePopulationData();
    ...
    console.log("population is null");
  }
  ...
  if (!shownGround && grounds && grounds.length > 0) {
    grounds[0].show();
  }
  if (panX < 0) {
...
function drawBrain() { //show the brain of whatever genome is currently showing
  ...
  if (runBest && population.bestPlayer.brain != null) {
    population.bestPlayer.brain.drawGenome(startX, startY, w, h);
  } else
  if (humanPlaying) {
    showBrain = false;
  } else if (showBestEachGen &&  genPlayerTemp && genPlayerTemp.brain != null) {
    genPlayerTemp.brain.drawGenome(startX, startY, w, h);
  } else if (!fightMode && currentBestPlayer && currentBestPlayer.brain != null) {
...
function writeInfo() {
  ...
  if (runBest && population && population.bestPlayer) {
    text("Score: " + population.bestPlayer.score, 650, 50); //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<replace
    text("Gen: " + population.gen, 1150, 50);
  } else {
    if (showBest && population && population.players[0]) {
    ...
    } else if (fightMode && population && population.bestPlayer && humanPlayer) {
      text("Your Score: " + humanPlayer.score, 100, 50);
      text("Enemy Score: " + population.bestPlayer.score, 850, 50);
    } else if(currentBestPlayer && population) {
...
async function getNewWorld(gen) {
  console.log("Inside getNewWorld");

  if((gen + 1 ) % 3 == 0 && (gen != 0)) {
    console.error("Generation 2x reached, need to post data to server");
    console.log(generatedGroundGAN[0])
    let newMap = await postData('http://localhost:3000/modifyground', {groundData: generatedGroundGAN[0], generationDeathData: generationDeathData}).then((data) => {
      console.log("THIS IS THE DATA IN THE GET NEW WORLD FUNCTION: ", data);
      return null;
    }).catch((error) => {
      console.error(error);
    });
    ...
    if(newMap) {
      ...
      generationDeathData = {};
      console.log("THIS IS THE NEW MAP: ", newMap);
      return newMap;
    }
  }
  return null;
...
async function preparePopulationData() {
  let newTempData = {}

  newTempData.players = [];

  for (var i = 0; i < population.players.length; i++) {
    newTempData.players.push(population.players[i].cloneForData());

  }

  if(population.bestPlayer != null) {
    newTempData.bestPlayer = population.bestPlayer.cloneForData()
  }
  newTempData.gen = population.gen;
  populationData[population.gen] = newTempData;
}

async function saveGenerationData(params) {
  console.log(JSON.stringify(populationData));
  if ((params + 1) % 15 == 0 && (params !=0)) {
    saveValidationGround(groundTemplate);
    // generationgenuned - generation (generated un edited)
    // generationreguned - generation (regular un edited)
    // generationreged - generation (regular edited)
    // generationgened - generation (generated edited)
    let response = await fetch('http://localhost:3000/generationflat', { 
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({populationData: populationData, populationDeathData: generationDeathData})
    });
...
async function awaitNewWorlds() {
  console.log("New World!!!");
  pause = true;

  let data = await saveGenerationData(population.gen).then((data) => {
  // let newMap = await getNewWorld(population.gen).then((data) => {
    console.log("THIS IS THE DATA: ", data);
    if (data) {
      generatedGroundGAN[0] = data;
      location.reload();
      return data;
      // refresh browser
  ...
  // groundTemplate.randomizeGround();
  if (newMap) {
    console.log("Generating new ground from server");
  }
  else {
    console.log("Generating new ground from GAN");
  }
  // while (groundTemplate.isTooSteep()) {
  //   groundTemplate = new Ground();
  //   groundTemplate.randomizeGround();
...
async function newWorlds() {
  console.log(population)
  return new Promise(async (resolve, reject) => {
    await awaitNewWorlds();
    resolve();
  });
}
