class Ground {
  ...
  async randomizeGround() {
    ...
    console.log(this.generateJsonOfGround());
  }

  generateJsonOfGround() {
    let groundJson = {
        segments: [],
        grassPositions: [],
    ...
    for (let i = 0; i < this.vectors.length - 1; i++) {
        let currentVector = this.vectors[i];
        let nextVector = this.vectors[i + 1];
        let deltaX = nextVector.x - currentVector.x;
        let deltaY = nextVector.y - currentVector.y;
        let length = sqrt(deltaX * deltaX + deltaY * deltaY);
        let angle = atan2(deltaY, deltaX);
        angle = degrees(angle); // Convert to degrees if needed

        groundJson.segments.push({
            x: currentVector.x,
            y: currentVector.y,
            length: length,
            angleToNextVector: angle
        });
    }

    for (var g of this.grassPositions) {
      groundJson.grassPositions.push(g);
    }
    return groundJson;
  }

  generateGroundFromJson(groundJson) {
    this.vectors = [];
    this.grassPositions = [];
    this.startingPoint = groundJson.startingPoint;
    for (let i = 0; i < groundJson.segments.length; i++) {
        let segment = groundJson.segments[i];
        let x = segment.x * SCALE;
        let y = segment.y * SCALE;

        this.vectors.push(new Vec2(x, y));
    }

    if(this.vectors.length < 2) {
      console.log("Ground has less than 2 points, exiting");
      return;
    }
  
    for (let g of groundJson.grassPositions) {
  ...
  show() {
    ...
    for (var i = 0; i < this.vectors.length - 3; i++) {
      if (this.grassPositions[2 * i] != -1 && this.grassPositions[2 * i] < grassSprites.length) {
      ...
      if (this.grassPositions[2 * i + 1] != -1 && this.grassPositions[2 * i + 1] < grassSprites.length) {
